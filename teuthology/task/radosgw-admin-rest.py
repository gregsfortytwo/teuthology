...
def task(ctx, config):
    ...
    (ret, out) = rgwadmin_rest(admin_conn, ['log', 'list'])
    assert ret == 200
    assert len(out) > 0
    ...
    for obj in out:
        ...
        (ret, log) = rgwadmin_rest(admin_conn, ['log', 'show'], {'object' : obj})
        ...
        (ret, out) = rgwadmin_rest(admin_conn, ['log', 'rm'], {'object' : obj})
        assert ret == 200
    ...
    (ret, out) = rgwadmin_rest(admin_conn, ['garbage', 'list'])
    assert len(out) > 0

    # TESTCASE 'garbage-process', 'garbage', 'process', 'manually collect garbage'
    (ret, out) = rgwadmin_rest(admin_conn, ['garbage', 'process'])
    assert ret == 200
    ...
    (ret, out) = rgwadmin_rest(admin_conn, ['pool', 'rm'], {'pools' : default_pool})
    assert ret == 200

    # now list the pools
    (ret, out) = rgwadmin_rest(admin_conn, ['pool', 'list'])
    assert len(out) == 0

    # TESTCASE 'pool-add', 'pool', 'add', 'make default pool available', 'succeeds'
    (ret, out) = rgwadmin_rest(admin_conn, ['pool', 'add'], {'pools' :  default_pool})
    assert ret == 200
    ...
    (ret, out) = rgwadmin_rest(admin_conn, ['zone', 'info'])
    assert len(out) > 0
